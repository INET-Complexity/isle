...
class ReinriskModel():
    def __init__(self, damage_distribution, expire_immediately, cat_separation_distribution, norm_premium, \
                category_number, init_average_exposure, init_profit_estimate):
            ...
            if len(categ_risks) > 0:
                ...
                if self.expire_immediately:
                    expected_profits += (self.norm_premium - (1 - scipy.stats.poisson(1 / self.cat_separation_distribution.mean() * \
                                        mean_runtime).pmf(0)) * self.damage_distribution.mean()) * average_exposure * len(categ_risks)
                else:
                    expected_profits += (self.norm_premium - mean_runtime / self.cat_separation_distribution.mean() * self.damage_distribution.mean()) * average_exposure * len(categ_risks)
            else:
                average_exposure = self.init_average_exposure
                expected_profits += 0
            var_per_risk = self.getPPF(self.var_tail_prob) * average_exposure
            necessary_liquidity += var_per_risk * len(categ_risks)

            print("RISKMODEL: ", var_per_risk, " = PPF(0.02) * ", average_exposure, " vs. cash: ", cash, "TOTAL_RISK_IN_CATEG: ", var_per_risk * len(categ_risks))
